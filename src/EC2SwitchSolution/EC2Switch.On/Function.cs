...
namespace EC2Switch.On
{
    public class Function
    {
        ...
        public async Task<ApiGatewayResponse> FunctionHandlerAsync(Parameter parameter, ILambdaContext context)
        {
            ...
            var response = await app.StopAsync(parameter.Request);

            return new ApiGatewayResponse
            {
                StatusCode = response.HttpStatusCode,
                Headers = ApiGatewayResponse.DefaultHeaders,
                Body = JsonConvert.SerializeObject(response, formatting: Formatting.None),
            };
        }

        public class Parameter
        {
            public StopInstancesRequest Request { get; set; }
        }
    }
}
